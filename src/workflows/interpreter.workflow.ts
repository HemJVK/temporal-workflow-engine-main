...
async function determineNextNode(
  ...
  // --- A. CONDITIONS (Multi-Rule or Binary) ---
  if (node.type === 'logic_condition') {
    // 1. New Format: Multi-Rule Router (generated by AI Helper)
    if (node.params?.rules && Array.isArray(node.params.rules)) {
      console.log(`[Flow] Evaluating ${node.params.rules.length} rules for condition node: ${node.id}`);
      for (const rule of node.params.rules) {
        if (evaluateConditionString(rule.condition, state)) {
          console.log(`[Flow] Rule Matched: ${rule.condition} -> Target: ${rule.target}`);
          // Target could be an ID or a branch key
          return node.branches?.[rule.target] || node.branches?.[rule.id] || findNodeIdByAlias(rule.target, fullPayload) || null;
        }
      }
      return node.branches?.['default'] || node.next || null;
    }

    // 2. Legacy Format: Binary True/False
    const { variable, operator, value } = node.params;
    const actualValue = findValueAnywhere(variable, state);
 ...
 */
/**
 * Evaluates a complex condition string like "severity == 'Normal' AND category != 'Lost Device'"
 */
function evaluateConditionString(condition: string, state: WorkflowState): boolean {
  if (!condition || typeof condition !== 'string') return false;
  console.log(`[Logic] Evaluating condition: ${condition}`);

  // Simple parser for AND/OR
  ...
  for (const part of parts) {
    ...
    // Evaluate individual comparison: "severity == 'Normal'"
    const match = part.match(/([a-zA-Z0-9_.]+)\s*(==|!=|>|<|>=|<=|contains)\s*(['"]?.*?['"]?)$/);
    if (match) {
      const [_, variable, operator, targetValueRaw] = match;
      const targetValue = targetValueRaw.replace(/['"]/g, ''); // strip quotes
      const actualValue = findValueAnywhere(variable, state);
      const isMatch = compareValues(actualValue, operator, targetValue);

      if (currentOp === 'AND') result = result && isMatch;
      else if (currentOp === 'OR') result = result || isMatch;
    }
  }
...
/**
 * Searches for a variable in the entire state, including nested objects from AI Agent outputs.
 */
function findValueAnywhere(variable: string, state: WorkflowState): unknown {
  // 1. Try direct/dot-notation resolution
  const direct = resolveStateValue(state, variable);
  if (direct !== undefined) return direct;

  // 2. Search inside all state keys (nodes) for the property
  for (const key of Object.keys(state)) {
    const nodeOutput = state[key];
    if (nodeOutput && typeof nodeOutput === 'object') {
      // If the node output has the property directly (e.g. state.ai_agent_1.severity)
      if (variable in (nodeOutput as any)) {
        return (nodeOutput as any)[variable];
      }
    }
  }
...
/**
 * Tries to find a node ID by its name/alias if branches are missing
 */
function findNodeIdByAlias(alias: string, payload: WorkflowPayload): string | null {
  const normalizedAlias = alias.toLowerCase().replace(/_/g, ' ');
  for (const [id, node] of Object.entries(payload.steps)) {
    if (node.name.toLowerCase().includes(normalizedAlias) || id.includes(alias)) {
      return id;
    }
  }
  return null;
